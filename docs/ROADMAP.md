# RoundFlow Phase 1 — Backend Roadmap

> Linear-ready delivery plan. Every checkbox is designed to become **one Linear
> ticket** (roughly one PR's worth of work).
>
> **Status legend:** 🔴 Not Started · 🟡 In Progress · ✅ Done
>
> **Ticket ID:** `BE-M<milestone>-<nn>`
...
### Definition of Done
- All required steps (1, 3, 4, 5, 6, 7, 8, 9, 10, 11) completable via the API; `setupCompleted` flips true; the wizard locks (403) after completion; step 2 is a deferred stub; step 5 (Message Templates) is required; step 12 is the read-only review.
...
### Definition of Done
- Admin can add a customer+property (assigned or unassigned), find it in the list, open the detail, edit it, and attach a service plan; changing a property's frequency automatically reassigns it to the correct round. Covers FR-CUST-1..6, FR-FREQ-1..6.
...
- [ ] **[BE-M3-01]** Visit generation cron — nightly job; for each ACTIVE round, check each property's `ServicePlan.nextDueDate`; generate the next cycle of `Visit`s; respect paused properties, archived rounds, CANCELLED customers, working days, and per-property frequency. **New visits start `technicianId=null`** (per-occurrence rule, SRS FR-ROUND-10). **Next up.** `labels: backend, visits`
...
## M4 — Today's Work · 🟡 In Progress
**Goal:** The office can monitor the live workday and intervene (reassign, push missed, close day).
**Depends on:** M3

> Backend complete (2026-08-12). All five BE tickets delivered in `feat/BE-M4`.

- [x] **[BE-M4-01]** Today's Work aggregate — per-round progress + KPI tiles (Screen 12). `labels: backend, todays-work`
- [x] **[BE-M4-02]** Round Details panel data — per-property job status + issue flags (Screen 13). `labels: backend, todays-work`
- [x] **[BE-M4-03]** Reassign Technician — move remaining jobs to another tech + note + notify flag (M15). `labels: backend, todays-work`
- [x] **[BE-M4-04]** Push Missed Jobs — move unfinished visits to a chosen date. `labels: backend, todays-work`
- [x] **[BE-M4-05]** Close Day — mark remaining scheduled visits + generate summary; `lastClosedDate` guard prevents re-close on the same day. `labels: backend, todays-work`

### Definition of Done
- The workday API reflects live visit statuses; admin can reassign a round's remaining jobs, push missed jobs, and close the day. Covers FR-VISIT-3/4/5.
...
## M6 — Payments & Debt Board · 🟡 In Progress
...
> Invoicing + Debt Board backend delivered (2026-08-12). GoCardless + Stripe integrations are Phase 2 dependencies. `Customer.holdNextClean` migration added for Hold Next Clean KPI. Payment link uses a Stripe Phase 2 placeholder URL (`https://pay.roundflow.app/i/{invoiceNumber}`).
...
- [x] **[BE-M6-04]** Invoice — generate / preview / send; Resend email delivery; `POST /invoices`, `GET /invoices/:id/preview`, `POST /invoices/:id/send` (feat/invoicing, 2026-08-12). `labels: backend, invoicing`
- [x] **[BE-M6-05]** Debt Board — 7 buckets (INVOICE_SENT, DUE_BEFORE_CLEAN, FAILED_GC, SEVEN_DAYS_OVER, FOURTEEN_DAYS_OVER, ON_HOLD, BAD_DEBT); KPI tiles; Bad Debt + Hold flags (feat/debt, 2026-08-12). See `docs/Handoffs/DEBT_BOARD_HANDOFF.md`. `labels: backend, debt`
- [x] **[BE-M6-06]** Send Payment Reminder (SMS/WhatsApp/Email via channel param) + Send Payment Link (email + Stripe placeholder); `POST /debt/:id/remind`, `POST /debt/:id/payment-link` (feat/debt, 2026-08-12). `labels: backend, debt`
...
## M7 — Complaints & Reporting · 🟡 In Progress
...
> Reports & Activity Log backend delivered (2026-08-12). `ActivityLog.actorId` + `actorRole` migration added; `logActivity` wired into properties, rounds, and today endpoints (feat/activity-logging). Complaints backend not yet started.
...
- [x] **[BE-M7-03]** Reports aggregates — revenue chart, completed visits/rounds, technician KPIs (efficiency, revenueImpact), visit history; `GET /reports/summary|revenue|technicians|visits` with `period` filter (feat/reports, 2026-08-12). See `docs/Handoffs/REPORTS_HANDOFF.md`. `labels: backend, reports`
- [x] **[BE-M7-04]** System Activity Log — `logActivity` helper on `createReportsService`; `ActivityLog.actorId/actorRole` migration; wired into POST /properties, PATCH/PUT/POST /rounds/*, POST /today/close; `GET /reports/activity` with optional type filter (feat/reports + feat/activity-logging, 2026-08-12). `labels: backend, reports`
...
### Definition of Done
- All settings sections editable via the API; **one full round cycle runs end-to-end without manual workarounds** (the Phase 1 success criterion); production hardening (CORS, logging, auth checks, trigger migration) applied.
...
- [x] `Customer.holdNextClean Boolean @default(false)` — migration `20260812000001_customer_hold_next_clean`; powers Hold Next Clean KPI on the debt board.
- [x] `ActivityLog.actorId String?` + `actorRole String?` — migration `20260812000002_activity_log_actor`; enables "by Admin" attribution in the activity feed.
...
- Route optimisation / GPS tracking (Phase 3 per the brief).
- Advanced analytics / BI.
- Customer-facing portal.
- First-class **`RoundOccurrence`** model (Phase 1 derives occurrences from Visits — see SDS §3.4).
- **Mobile Technician App** — React Native native app (iOS + Android); Phase 1 delivers the BE endpoints (`BE-M5-*`) only. (DP-MOBILE resolved.)
...
| **Notification provider** — DP-NOTIF: push notifications required (FR-MOBILE-9) but provider undecided (Firebase or equivalent); schema for notification entity undefined | **M5** (BE-M5-07) |
| **Skip-reason enum** (#7) — currently free `String` | **M5** (BE-M5-04) |
| **Per-round assignment history** (#9) | **M3/M4** (Screen 30 "Recent Activity") |
| ~~**Mobile completion delivery** (native vs mobile web) — DP-MOBILE~~ | ✅ **Resolved 2026-07-16** — React Native native app (iOS + Android); mobile FE deferred to Phase 2 |
