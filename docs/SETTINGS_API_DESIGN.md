...
*Design only — no code or migration is proposed to run yet. Grounded in `designFindings.md` Screens 23/24/32–36 + M16/M17 (audited 2026-07-08), the current Prisma schema, and the `SetupService` OCP pattern (`profileId`-first, singleton-backed).*
...
| **35** Technician Mgmt | `Technician[]` | ⚠️ partial — step 6 creates invite-pending | name/email/default-area (see §6) |
| **32** Payment Setup | *(new)* provider connections + rules | ❌ new | new fields/model (see §3, §6) |
...
- **Gap:** the screen shows cycle as *Week / 2-week / 3-week / 4-week* but the schema stores `defaultCycleLength` as **Int days**. Recommend keep Int days; UI maps weeks→days (7/14/21/28). Flag for confirmation.
...
- **Coexists** with the wizard's full-replace `saveServices` (wizard bulk-seeds; Settings edits individually). **New field:** `isDefault` (M16/M17 "Default — pre-selected on new jobs" toggle). **Delete guard:** once `ServicePlan`/`Visit` reference services (post-M3), block or soft-delete.
...
- **New field:** `description`. **Delete guard:** block if a `Round`/`Property` references the area (or force reassign).

### 35 — Technician Management *(per-item — see §6 for the conflict)*
...
- `POST /settings/technicians` → invite-pending create with admin-entered `name` + `phone` + optional `defaultAreaId`.
- `PATCH /:id`, `DELETE /:id`.
...
- `POST /settings/payment/:provider/connect` → **provider-agnostic** `{ status, connectUrl? }` (see §6 for why this shape).
...
Legend: **[P1-req]** goes in the next migration · **[P1-nice]** Phase 1 if time permits · **[P2]** defer to Phase 2.

| Target | New field / model | Type | Phase | Rationale |
|---|---|---|---|---|
| `Technician` | `name` | `String?` **nullable** | **[P1-req]** | Screen 35 collects Full Name at add-time; nullable so it never blocks the invite/accept path (§6). |
...
| `BusinessSettings` | `debtHoldEnabled` | `Boolean @default(false)` | **[P1-req]** | Gates payment-hold visit generation (FR-VISIT-1) — behavioural. |
| `BusinessSettings` | `paymentRule` | enum `PaymentTiming` (`COLLECT_AFTER_VISIT`…) | **[P1-req]** | Drives the M6 collection trigger. |
| `BusinessSettings` | `vatInInvoices` | `Boolean @default(false)` | **[P1-nice]** | Invoice detail (distinct from `vatRegistered`). |
...
| `BusinessSettings` | `defaultCleanMethod` | existing clean-method enum (`TRADITIONAL`/`WATER_FED_POLE`) | **[P1-nice]** | Default for new plans; not blocking. |
...
| *(new)* `PaymentProviderConnection` | `provider, status, externalRef` | model | **[P1-nice]** | Cleaner than token columns on the singleton; Phase 1 can start with two `*Connected` booleans (see §6). |
| *(new)* `TechnicianInvite` | invite token | model | **[P1-req]** (M5) | Required for mobile onboarding, not for Settings alone. |
| *(new)* `MessageTemplate` | `key, channel, body` | model | **[P2]** | GHL owns messaging in Phase 2 — **do not** create a local table now (§6). |

**Concrete "next migration" (P1-req only):** `Technician.name String?`, `BusinessSettings.debtHoldEnabled`, `BusinessSettings.paymentRule` (+ `PaymentTiming` enum). Everything else is P1-nice/P2 and can be batched later. `TechnicianInvite` lands with M5, not with the Settings migration.
...
## 5. Open questions (resolve before migration)

1. **Cycle representation** — days (`Int`) with UI mapping, or add a weeks/frequency field? (Recommend: keep days.)
2. **Payment provider connect** — real OAuth in Phase 1, or a stub/manual-token until GHL wiring? (Affects whether `PaymentProviderConnection` is P1-req.)
3. **Payment Setup vs. wizard step 2** — the Settings screen is far richer than the deferred step-2 stub. Do we promote step 2 to real in the wizard, or only expose payment config via Settings? (Recommend: Settings-only for now; wizard step 2 stays deferred.)
4. **SMS Templates** — pure GHL deferral now, or editable local store behind a service interface? (Recommend: deferral stub; §6.)
5. **Service/Area delete guards** — behaviour once `ServicePlan`/`Round` references exist (block vs. soft-delete vs. reassign).
6. **Technician identity** — confirm the name-as-label resolution in §6.
...
- **Invite flow unchanged:** `technicianService.invite(profileId, { name, phone, defaultAreaId })` creates the invite-pending row; a `TechnicianInvite` token (lands with M5) carries the email; mobile acceptance links a `Profile` to the **same** `Technician` by token. Nothing in that path requires a field only the mobile flow can produce → **no breakage**.
...
- **Next migration = P1-req only:** `Technician.name?`, `BusinessSettings.debtHoldEnabled`, `BusinessSettings.paymentRule` (+ `PaymentTiming`). Defer the rest.
...
*Resolutions to the §5 open questions (approved 2026-07-08). These govern the `settings-schema-p1` migration and Phase-1 scope.*
...
**Migration impact (`settings-schema-p1`, P1-req):** `Technician.name String?`; `BusinessSettings.debtHoldEnabled Boolean @default(false)`; `BusinessSettings.paymentRule PaymentTiming?`; `BusinessSettings.gocardlessConnected Boolean @default(false)`; `BusinessSettings.stripeConnected Boolean @default(false)`; new enum `PaymentTiming { COLLECT_AFTER_VISIT, COLLECT_BEFORE_VISIT, COLLECT_ON_DATE }`. **Decisions 1, 3, 4 add no schema.** Decision 5 is enforced in the **service layer**, not the schema.
